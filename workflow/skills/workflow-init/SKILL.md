...
`docs/PATTERNS.md` is NOT placed here — it is generated by `project-init-write` from `patterns_include`. Skip.
...
Inject ONLY the reference line to `workflow-instructions.md`. Do NOT inline `workflow-config.md`, `workflow-git.md`, `workflow-infra.md`, or `workflow-smoke.md` — those are loaded on-demand by skills, not every turn.
